...
- 新增「AI 余额」：填一个大模型网关的 API Key（内置 LLM Gateway 预设，也可自定义接口或直接粘贴 curl），余额就显示在侧边栏顶部，带剩余百分比和进度条；扩展图标角标显示剩余百分比（金额位数太多，角标只能放 4 个字），鰠标悬停可看完整金额；可选在 Octo 输入框旁显示，低于设定阈值时转红。密钥只存在本机，不同步、不进网页。
